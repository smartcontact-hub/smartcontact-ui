...
| **Experimento en LOCAL: la escala al tamaño de Aura (16px por rem)** | Rafa, 2026-09-14: «apuntar este experimento para local». Aura no tiene escala; la del Kit es la de PrimeOne (34 pasos, idénticos) y cada paso se llama como un rem de Aura (`scale/0-375` = 0,375rem) pero vale rem×14, así que la app pinta un 12,5 % más pequeño que primeng.dev (campo 29,5 contra 31). Llevarla a rem×16 agranda un 14 % todo lo que usa la escala: componentes, 1.280 usos de `--sc-spacing-*` y marcos de Figma. **Antes de probarlo**: medido el 2026-09-14 que cambiar un valor de `scale/*` en Figma rompe EN SILENCIO (`scaleSuffix` nombra por valor/14 → `0-375` pasa a `0-429`, `--sc-spacing-0-375` apunta a nada, y `tokens:import`, `tokens:parity` y `tokens:guard` salen en verde). Hace falta nombrar por la clave del export y un gate rojo para `var(--sc-scale-*)` sin definir. Enseñarlo en local lado a lado antes de tocar Figma |
