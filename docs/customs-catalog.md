...
- **Excepción — iconos de marca**: glifos sin equivalente Material (p.ej. **GitHub**) se resuelven con un `<svg>` inline `fill="currentColor"` en el consumer, NO con `<sc-icon>`.
  - **Dentro de un `sc-button`** (2026-09-23): el SVG va en `<ng-template #scButtonIcon>`, que el botón pasa a PrimeNG como su plantilla `icon`; así toma la forma de solo icono y la talla del resto. El SVG mide `1em` y trae la geometría de la librería Smart-Contact Icons (ya ampliada 24/18). Primer caso: «documento con análisis» en la toolbar del reproductor del Supervisor. `iconOnly` de PrimeNG no sirve para esto: en 22.1 no pone la clase `p-button-icon-only` (medido).
...
- **SC**: input opt-in `[flush]` (default `false`) en:
  ...
  - **`<sc-form-section-nav>`**: ⚠️ **cambió el 2026-09-14 (DD-100)**. Ya no es un panel gris con el icono en su chip: se ve como el índice de Contact Center (marco Figma `393:12565`, «sc-form-section-nav (pure-sc)»): sin panel ni borde, icono desnudo a 20, etiqueta `Body/body-regular`, activo con `--sc-bg-hover` y semibold. **El label sigue envolviendo, no se recorta** (DD-52), vigilado por `e2e/supervisor/form-section-nav-legibility.spec.ts`. La generación anterior (`12277:4818`) queda sustituida; la librería del DS no tiene componente de índice (`figma-pendiente.md` §5).
