...
| 2026-07-21 | Page `❖ Breadcrumb` — frame `Current state · propuesta` (node `13890:157`) | Propuesta del estado «tramo actual más oscuro» (`#4F5663` el actual, `#8F97A3` los padres, mismo peso). **El componente maestro NO se tocó.** | El breadcrumb debe marcar dónde estás; PrimeNG pinta todos los tramos iguales. Ver `customs-catalog §2.12`. | Claude (MCP, pedido por Rafa) — *fila añadida a posteriori en la s22: la escritura se hizo en la s21 y no se registró aquí* |
| 2026-07-22 | Page `Flujos` — frame `Lienzo de página — Current state · propuesta (código S22)` (node `13920:4298`), a la derecha del maestro `SmartContact AED` | Propuesta de **retirar el gris del lienzo de página** (slate-50 → blanco) dejando que la separación la haga el borde de la card, que ya existe en el maestro. Incluye comparativa A/B y los números de los dos lados. **El maestro NO se tocó** (comprobado: la escritura solo crea nodos nuevos en espacio libre, x=6500). | DD-34: el gris mide 1.063:1 contra la card en claro —invisible— y **1.00:1 en oscuro**, donde es literalmente el mismo color; el borde mide 1.34:1 y vale en los dos temas. Decide diseño (Rafa). | Claude (MCP oficial, pedido por Rafa) |
| 2026-07-22 | Variables de Primitive/Semantic/Component (~2000: numéricas y de color) | **Migración de scope + rebind tipográfico + descripciones.** NÚMEROS: 28 pesos de componente crudos (400/500/600/700) rebindeados a `typography/font/weight/{regular,medium,semibold,bold}` (mismo valor, cambio visual 0; los 6 aliased heredan por su hub); 943 floats pasados de `ALL_SCOPES` a su campo (font-size 51, font-weight 39, line-height 9, corner-radius 154, width/height 238, gap+padding 505, stroke 4, effects 2, opacity 1); `scale/*` (34) suelta `FONT_VARIATIONS` y queda W/H+Gap+Stroke; 67 tokens abstractos (focus-ring-offset/position) a `scopes=[]` (fuera del picker). COLOR: 1128 scopeados por rol (background a Fill, border/ring a Stroke, icon a Shape, texto/placeholder a Text); acentos semánticos (38) y paleta cruda quedan broad por ser polimórficos. TYPOS: `typography/font/line hieght` a `typography/line/height` (reactiva `tokens:type-parity`), `togglebutton/*/zadding` a `padding`, pesos a minúscula. DESCRIPCIONES: 201 "cómo usar" (59 fundaciones + 142 vocabulario semántico). | Que el picker ofrezca solo lo de cada campo (dejar de rebuscar entre todas), el texto quede gobernado por la escala Custom, y que el gate de line-height vuelva a comprobar drift. Migration-safe: additivo, rebind mismo-valor, renombrar no desancla (bindings por ID). Verificado en local: `tokens:type-parity` 15/15, `parity` y `guard` en verde, 0 diff de código propio (el único cambio de color en código es el tema info blue a sky, aparte, vía PR #17). | Claude (MCP, pedido por Rafa) |
| 2026-08-05 | Variables — colección `Primitive` (+ `Semantic`/`Component Color Scheme`) | **Ancla blanca del grafo.** Añadido **1 primitivo blanco `slate/0`** (= `#ffffff`) y relinkeados **18 valores crudos `#ffffff` → alias**: `surface/0` (L **y** D → `slate/0`), `primary/contrast/color` (L→`slate/0`) y 15 `button/{info,success,warn,help,danger}/{,hover,active}/color` (L→`slate/0`). Los modos **Dark** de esos 16 ya aliasaban (sky/950, red/950, surface/900…); solo el blanco de **claro** estaba crudo. | No existía primitivo blanco (la rampa `slate` de Figma empezaba en 50; el código sí tiene `--sc-color-slate-0: #ffffff`), así que `surface/0` —único paso de la rampa sin alias— y los blancos de contraste horneaban `#ffffff` a pelo. Ahora aliasan: grafo linkeado por el lado opaco. **0 cambio visual** (todo sigue `#ffffff`), verificado por re-escaneo independiente (**0 tokens opacos crudos restantes**, de 17). No se tocan los 81 translúcidos (recetas `color-mix`, inexpresables como alias en Figma) ni los 21 transparentes. Alinea la rampa de Figma con la del código. | Claude (MCP, delegado por Rafa) |
| 2026-08-05 | Variable `text/muted/color` (Semantic Color Scheme) | **Modo claro** re-apuntado `slate/500` → **`slate/600`** (`#8f97a3` → `#6f7784`). **Modo oscuro sin tocar** (`#a1a1aa`). | Accesibilidad **AA**. slate-500 sobre blanco ≈ **3:1 (falla AA)**; slate-600 ≈ **4.5:1 (AA)** y sigue leyéndose como muted (jerarquía intacta). El panel *Check designs* marcaba estos textos porque el token no pasaba AA. Figma arrastraba aún el valor de serie de PrimeNG; el código ya tenía `--sc-text-secondary` = slate-600 por AA (§1.5), así que esto **alinea Figma con lo que ya se envía en producción**. Oscurece un pelín el texto muted en claro en todos los consumidores, a propósito. | Claude (MCP, pedido por Rafa) |
