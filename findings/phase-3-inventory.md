...
**12 tamaños de la réplica no aparecen en el original.**

El más llamativo es el primero: `0.803572vw` = **11.70px con 18 usos**. El original no lo
declara nunca. Su base es `* { font-size: 0.8vw }` = **11.65px**, que la réplica también
tiene (14 usos). Los dos conviven para lo que casi seguro es lo mismo: 11.70 salió de
redondear a mano `0.8 × 14.56 = 11.648` cuando la réplica medía en px. No es una medida,
es un redondeo mío que se propagó a 18 sitios.

Lo mismo, más pequeño: `0.728022vw` (10.60px) contra el `0.729vw` (10.61px) del original.

**No se tocan.** Son 0.05px y 0.01px: por la tolerancia del encargo eso es RUIDO
(&lt;0.25px), y el ruido no se toca. Queda registrado para que exista el dato, no para
justificar una pasada de limpieza.

Los grandes —31.90, 26.40, 26.00, 21.30, 20.50— son las tarjetas KPI del dashboard. Se
midieron en vivo, no se leyeron de su CSS, así que su ausencia aquí puede significar tres
cosas distintas: que el original los compone (por ejemplo con el `*` global), que viven en
un chunk que este censo no alcanza, o que están mal. **Sin verificar**: distinguirlo exige
medir el original en vivo.
