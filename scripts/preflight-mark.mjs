 ...
 * ahora es un fichero que escribe el último paso de `preflight` / `preflight:scope -- --run`,
 * y que lee el hook de `git push` (`scripts/hooks/bash-guard.mjs`).
 *
...
import { estadoTrasLaCadena } from './preflight-rebase.mjs';
...
export function escribirMarca(cwd, carril) {
  // main pudo avanzar DURANTE la cadena. Si funde limpio se avisa y se sigue; si daría conflicto,
  // lo que pasó el preflight no es lo que acabaría en main y hay que repetirla (2026-09-11).
  const rebase = estadoTrasLaCadena(cwd);
...
export function estadoPreflight(cwd) {
  ...
  // Y si main avanzó desde entonces, solo importa cuando el merge daría CONFLICTO: eso sí acaba
  // «en conflicto» en `ci:verdict` y obliga a rebasar y repetir la cadena (LEARNINGS #7).
  const rebase = estadoTrasLaCadena(cwd);
