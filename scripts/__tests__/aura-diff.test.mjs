...
test('normalización: el mismo valor escrito de cuatro formas compara igual', () => {
  ...
  // Abreviatura de lados: el mismo relleno escrito de varias formas.
  assert.equal(normalizeValue('3.5px 3.5px'), '3.5px');
  assert.equal(normalizeValue('4px 8px 4px 8px'), '4px 8px');
  assert.equal(normalizeValue('0 0 7px 0'), '0 0 7px');
  assert.equal(normalizeValue('2px 6px'), '2px 6px');
});
 ...
 */
test('botón real: el relleno sale del Kit y un apartamiento de Aura lleva su motivo', async () => {
  // Contra el repo VIVO: no fija valores (cambian cuando Figma cambia), sino las relaciones que la
  // herramienta tiene que sostener con cualquier export.
  ...
  assert.equal(padding.aura, '6px'); // Aura 0.375rem
  assert.equal(padding.sc, padding.kit, 'el relleno del botón en código es el del Kit');
  assert.ok(['igual', 'kit', 'primeone'].includes(padding.reason), `motivo del relleno: ${padding.reason}`);

  // La fuente del md NO la toca el tema: sale igual que en Aura.
  assert.equal(row('--p-button-font-size').status, 'igual');

  // Toda diferencia marcada «accesibilidad» o «comentado» trae la nota que la justifica.
  for (const r of rows) for (const s of Object.values(r.schemes)) {
    if (s.reason === 'accesibilidad' || s.reason === 'comentado') assert.ok(s.notes.length > 0, `${r.key}: ${s.reason} sin nota`);
  }
});
